...
class PoseExtractor:
    def __init__(self, static_image_mode=False, model_complexity=1, min_detection_confidence=0.5):
        ...
        params:
            ...
            model_complexity: 0, 1, or 2 (2 is most accurate but slower)
...
if __name__ == "__main__":
    # Create a dummy test if run directly
    # You will need to drop a file named 'test_swing.mp4' in this folder to test
    video_file = "test_swing.mp4"
    
    if os.path.exists(video_file):
        extractor = PoseExtractor()
        data = extractor.process_video(video_file)
        
        # Save to JSON to verify output format
        with open("swing_analysis.json", "w") as f:
            json.dump(data, f, indent=2)
        print("💾 Saved results to swing_analysis.json")
    else:
        print(f"⚠️  Please place a '{video_file}' in this folder to test.")
